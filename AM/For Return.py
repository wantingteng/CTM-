...
# 追加逻辑
def append_return_data():
    new_file = select_file("请选择要追加的Return新数据文件")
    if not new_file:
        return
    mother_file = select_file("请选择Return母表文件")
    if not mother_file:
        return
    ...
    try:
        ...
        messagebox.showinfo("完成", f"Return 数据已成功追加并更新母表：\n{mother_file}")
...
def run_ui():
    root = tk.Tk()
    root.title("Return 数据追加工具（支持多Sheet）")
    canvas = tk.Canvas(root, width=400, height=180)
    canvas.pack()

    btn = tk.Button(root, text="追加 Return 数据到母表", command=append_return_data, height=2, width=30)
